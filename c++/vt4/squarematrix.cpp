...
SquareMatrix::SquareMatrix(const std::string& s)
{
    ...
    while (c!=']')
    {
        std::vector<unique_intele_ptr> rowvec;
        do
        {
            inputstrm >> number;
            ...
            if((c!= ',') && (c!= ']'))
               throw std::invalid_argument("Invalid init string");
            rowvec.push_back(IntElement(number).clone());
            col = col+1;
        }while (c != ']');

        //rivi luettu
        elements.push_back(std::move(rowvec));
...
SquareMatrix::SquareMatrix(const SquareMatrix& m)
{
    ...
    {
        std::vector<unique_intele_ptr> row_vector;
        for(auto& m_elem : m_row)
        {
            row_vector.push_back(m_elem->clone());
        }
...
SquareMatrix SquareMatrix::transpose() const
{
    ...
    sqm.elements = std::vector<std::vector<unique_intele_ptr>>(n);
    for(auto row = elements.begin(); row != elements.end(); row++)
    {
        ...
        {
            trans_row->push_back((*elem)->clone());
            trans_row++;
        }
    }
    return sqm;
